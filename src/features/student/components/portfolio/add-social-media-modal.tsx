...
import type { SocialMediaItem } from "../../schemas/portfolio.schema";
...
export function validateAndParseSocialUrl(
  ...
  // Ensure input looks like a URL or domain
  let urlString = trimmed;
  if (!/^https?:\/\//i.test(urlString)) {
    urlString = `https://${urlString}`;
  }
  ...
  // Parse path & extract username based on platform
  ...
  if (platformId === "linkedin") {
  ...
  } else if (platformId === "github" || platformId === "instagram" || platformId === "behance" || platformId === "dribbble" || platformId === "artstation" || platformId === "twitter") {
    ...
    if (segments.length >= 1) {
      // Handles /@username, /c/username, /user/username, /channel/xyz, etc.
...
export function cleanSocialMediaUsername(platform: string, input: string): string {
  ...
  // If input contains full URL patterns, parse and extract the username
  if (
  ...
  // Remove full URLs & common prefixes
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl max-w-[380px] w-full shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col h-[460px] max-h-[85vh]">
        ...
        {/* View 1: Platform Selection List with Search & Custom Scrollbar */}
        {!selectedPlatformId && (
          <div className="flex-1 min-h-0 flex flex-col p-3.5 overflow-hidden">
            ...
            {/* List of Platforms with Custom Smooth Slim Scrollbar */}
        ...
        {selectedPlatformId && selectedPlatform && (
          <form onSubmit={handleSave} className="flex-1 flex flex-col p-4.5 space-y-3.5">
            ...
            <div>
              ...
              {/* Helper text or real-time validation error */}
