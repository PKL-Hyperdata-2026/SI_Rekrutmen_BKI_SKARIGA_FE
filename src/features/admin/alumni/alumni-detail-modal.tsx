...
export function AlumniDetailModal({
  ...
  const validateFile = (file: File): boolean => {
    ...
    if (!allowedExtensions.includes(ext)) {
      toast.error("Format file tidak didukung. Harap unggah PDF, JPG, PNG, atau DOC/DOCX.");
      return false;
    }
  ...
  const handleUploadSubmit = async () => {
    ...
    if (!uploadCategoryId) {
      toast.error("Kategori berkas wajib dipilih.");
      return;
    }
    if (!uploadTitle.trim()) {
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3 animate-in fade-in duration-150">
                  ...
                  <div className="pt-2 flex items-center justify-end gap-2">
                    ...
                    <button
                      ...
                      disabled={isUploading || !selectedFile || !uploadCategoryId || !uploadTitle.trim()}
